...
def account(c, username='chloe', role='editor'):
...
def test_viewer_cannot_write_any_business_kind(client, kind):
    ready(client);member = account(client, role='viewer')
    shared = record(client).json()
    as_member(client)
    state = client.get('/api/state').json()
    assert state['workspace']['access'] == 'viewer' and not state['config']['can_edit']
    ...
    assert client.patch('/api/records/' + kind + '/' + shared['id'], json={'version': 1}).status_code == 403
...
def test_team_owner_controls_roles_membership_and_independent_teams(client):
    ...
    assert client.post('/api/teams/' + team['id'] + '/members', json={'username': 'chloe', 'role': 'viewer'}).status_code == 200
    assert set_role(client, member['id'], 'viewer').status_code == 200
    assert set_role(client, member['id'], 'editor').status_code == 409
    assert set_role(client, member['id'], 'owner', 2).status_code == 400
    as_member(client)
    assert client.post('/api/teams', json={'title': 'forged'}).status_code == 403
    assert set_role(client, member['id'], 'editor', 2).status_code == 403
    assert client.post('/api/teams/company/members', json={'username': 'kevin'}).status_code == 403
    ...
    client.headers['X-Cmp-Workspace'] = 'company'
    assert client.get('/api/state').status_code == 403
    assert client.get('/api/teams/company/members').status_code == 403
    client.headers['X-Cmp-Workspace'] = team['id']
    assert client.get('/api/state').json()['workspace']['access'] == 'viewer'
...
def test_legacy_schema_migration_preserves_data_versions_and_audits(tmp_path, monkeypatch):
    ...
    with app.db() as c:
        ...
        assert c.execute('SELECT role FROM memberships WHERE user_id=?', ('member',)).fetchone()['role'] == 'editor'
...
def test_membership_audit_failure_rolls_back_permission_change(client, monkeypatch):
    ...
    with pytest.raises(RuntimeError): set_role(client, member['id'], 'viewer')
    assert next(u for u in client.get('/api/teams/company/members').json() if u['id'] == member['id'])['team_role'] == 'editor'
