...
项目详情页提供“删除项目”。只有公司 Owner/Admin 或个人空间主人可操作，须输入完整项目名称并提交当前版本；项目已更新时需刷新后重新确认。

删除采用保留历史的方式：项目及其分区、笔记、项目待办、附件退出日常列表，并拒绝新增、修改和附件预览。附件原文件不删除，原有授权范围内的操作历史保留。财务流水与报销继续按账本权限管理；库存流水保留、只读并继续计入库存，避免改变账本盈亏与实际库存余额。界面暂不提供恢复入口。

### 沟通记录与附件

新增或编辑沟通记录时，可上传新文件或勾选同一项目已有的附件。一条记录可关联多份文件，同一份文件也可用于多条沟通记录；解除关联不会删除原文件。“附件”标签统一显示文件并标出关联的沟通记录，分区视图也包含该分区沟通引用的文件。上传附件时也可直接选择已有沟通记录，文件自动采用该记录的分区。

附件关联遵循项目权限。文件记录与笔记关联及双方审计在同一事务提交；上传重试不会重复创建文件或撤销用户主动解除的关联。已关联文件换项目前须先解除笔记关联。新增笔记的重试标识避免重复创建笔记及其待办。

### Google Drive 链接附件

在项目“附件 → 添加”中切换“Google Drive 链接”，可添加文件、文件夹或 Google 文档分享链接，填写显示名称并选择关联的沟通记录。链接与笔记关联及双方审计同一事务提交，重复请求不会重复创建或恢复已解除的关联。链接仅保存地址，不复制文件，不受本平台上传的 20 MB 限制；在新标签页打开，Google 自身的共享权限仍然适用，无需 OAuth 连接。

已粘贴在沟通正文中的 Drive 链接会自动显示卡片，也计入项目附件列表与数量。同一链接在多条正文中出现时合并显示，并保留来源笔记和分区筛选。此识别只使用当前已授权记录，不读取外部内容、不迁移或改写历史笔记；在来源正文中移除链接即可取消该来源的展示，显式添加的附件仍保留。
