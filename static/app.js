...
const labels = {home:'工作台',projects:'项目',finance:'财务',inventory:'库存',tasks:'待办',activity:'操作记录',users:'账号管理',team:'团队成员',security:'账号安全'};
const kindLabels = {warehouses:'仓库',products:'商品',stock_movements:'出入库记录',finance_profiles:'财务账本',subsections:'分区',projects:'项目',transactions:'财务记录',tasks:'待办',notes:'沟通笔记',files:'附件',user:'账号',workspace:'团队',membership:'团队成员',maintenance:'数据维护'};
...
function editor(kind,id) {
 if(kind!=='users'&&!canEdit()){readOnlyRecord(id);return;}
...
function userEditor(id) {
 const p=allAccounts.find(x=>x.id===id), teams=availableSpaces.filter(w=>w.kind==='team'&&w.access==='owner');
 const teamFields=id?'':select('team_role','加入团队后的权限',[[ 'none','仅创建个人账号'],['viewer','Viewer · 只读'],['editor','Editor · 可编辑']],teams.length?'editor':'none')+select('team_id','加入团队',teams.map(w=>[w.id,w.title]),teams[0]?.id);
...
function auditText(data) {if(!data)return '无';return Object.entries(data).filter(([k])=>k!=='id').map(([k,v])=>`${fieldLabels[k]||k}：${k==='source_import'?sourceLabel(v):k==='warehouse_id'?warehouseName(v):k==='product_id'?productName(v):k==='linked_transaction_id'?(find(v)?.title||'未关联'):k==='profile_id'?financeProfileName(v):k==='project_id'?projectName(v):k==='subsection_id'?sectionName(v):k==='linked_task_id'?(find(v)?.title||'未关联'):['owner_id','assignee_id'].includes(k)?person(v):v??'未填写'}`).join('\n');}
...
document.addEventListener('click',async e=>{
 ...
 try {
  if(!canEdit()&&(b.dataset.toggleTask||b.dataset.stockIn||b.dataset.stockOut)){toast('当前空间为只读权限');return;}
