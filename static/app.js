...
const kindLabels = {projects:'项目',transactions:'财务记录',tasks:'待办',notes:'沟通笔记',files:'附件',user:'账号'};
let me, records = [], people = [], config = {}, events = [], view = 'home', projectId = null, projectTab = 'notes', search = '', filter = '', taskFilter = '未完成';
let refreshTimer, toastTimer, lastSnapshot;
...
function editor(kind,id) {
 ...
 const record=id?find(id):null, data=record||{date:today(),project_id:projectId||'',currency:'USD',direction:'支出'};
 let content='';
 ...
 bindSubmit(async form=>{const body=Object.fromEntries(form);if(id)body.version=record.version;await api('/records/'+kind+(id?'/'+id:''),id?'PATCH':'POST',body);closeModal();await load();toast('已保存，更新人：'+me.name);});
}
...
const fieldLabels={title:'标题',name:'姓名',date:'日期',event:'事件',amount:'原币金额',usd_amount:'美元折算金额',currency:'币种',direction:'收支',payment_status:'支付状态',posting_status:'入账状态',booked_amount:'入账金额',payment_method:'付款形式',responsible:'负责人',category:'类别',note:'备注',description:'说明',body:'正文',contact:'联系人',status:'状态',progress:'进度',due_date:'截止日',project_id:'项目',owner_id:'负责人',assignee_id:'负责人',username:'账号',role:'角色',active:'启用',must_change:'需改密码',url:'链接'};
function auditText(data) {if(!data)return '无';return Object.entries(data).filter(([k])=>k!=='id').map(([k,v])=>`${fieldLabels[k]||k}：${k==='project_id'?projectName(v):['owner_id','assignee_id'].includes(k)?person(v):v??'未填写'}`).join('\n');}
...
function uploadForm() {openModal('上传项目附件',formWrap(projectSelect(projectId||'',true)+field('file','文件（最大 20 MB）','','file','required'),'上传到 Google Drive'));bindSubmit(async form=>{if(form.get('file').size>20*1024*1024)throw new Error('文件不能超过 20 MB');await api('/upload','POST',form);closeModal();await load();toast('文件已保存到 Google Drive');});}
...
document.addEventListener('click',async e=>{
 ...
 try {
  if(b.dataset.view){view=b.dataset.view;projectId=null;search='';filter='';closeModal();draw();window.scrollTo(0,0);}
  else if(b.dataset.new)editor(b.dataset.new);
  else if(b.dataset.edit){const r=find(b.dataset.edit);editor(r.kind,r.id);}
  else if(b.dataset.project){view='projects';projectId=b.dataset.project;projectTab='notes';draw();window.scrollTo(0,0);}
...
document.addEventListener('change',e=>{if(e.target.id==='filter'){filter=e.target.value;$('#page').innerHTML=renderPage();}});
